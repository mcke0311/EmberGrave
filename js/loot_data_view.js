...
(() => {
  ...
  function scenario(){difficulty=Number($('difficulty').value);const normalLevel=num('level',150);return {normalLevel,level:DATA.effectiveLevel(normalLevel,difficulty),mf:num('mf',10000),source:$('source').value};}
  function renderItems(){
    ...
    $('scenario').textContent=`Comparing the same ${D.sources[source].toLowerCase()}: ${DATA.DIFFICULTIES.map(d=>`${d.name} Lv ${DATA.effectiveLevel(normalLevel,d.id)}`).join(' / ')} · ${mf}% Magic Find · Chance of at least one copy per ${source==='chest'?'opening':source==='barrel'?'break':'kill'}. Sorting and eligibility use ${DATA.DIFFICULTIES[difficulty].name}. Difficulty raises source level (${DATA.DIFFICULTIES.map(d=>'+'+d.lvlAdd).join(' / ')}), changing the eligible loot pool; it does not directly multiply drop rates. Higher difficulty can make an early item stop dropping.`;
    $('itemHeaders').innerHTML='<th scope="col">Item / base</th><th scope="col">Level / slot</th>'+DATA.DIFFICULTIES.map((d,i)=>`<th scope="col" class="${i===difficulty?'active-difficulty':''}">${esc(d.name)}<small>Lv ${DATA.effectiveLevel(normalLevel,d.id)}</small></th>`).join('')+'<th scope="col">Brown power</th>';
