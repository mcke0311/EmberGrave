...
const MobileViews=(()=>{
  ...
  const key=root=>(Game.state?.player?.heroId||Game.state?.player?.name||'')+':'+root.dataset.kind;
  ...
  function release(root){remember(root);}
  ...
  return {mount,release,remember,restoreScroll,detail,back,isDetail,enabled};
})();
