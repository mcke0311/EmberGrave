...
export function createWeapon(item){
  ...
  if(category==='sword'||category==='dagger'){
    ...
    }support=[0,category==='spear'?.26:.30,0];
    ...
    rod(root,m.leather,[0,-.10,.2],[0,.10,.2],.031);
    for(const s of [-1,1]){
      const limb=new THREE.Group();limb.name=s>0?'BowUpperLimb':'BowLowerLimb';limb.position.set(0,s*.10,.2);limb.userData.side=s;root.add(limb);
      const local=([x,y,z])=>[x,y-s*.10,z-.2],points=[[0,s*.10,.2],[0,s*.23,.18],[0,s*.50,.08],[0,s*.63,-.07]].map(local);
      curved(limb,m.wood,points,.023);curved(limb,m.trim,points.map(([x,y,z])=>[x-.013,y,z+.012]),.005);
      rod(limb,m.metal,local([0,s*.56,.033]),local([0,s*.635,-.074]),.015);
      for(let i=0;i<3;i++)box(limb,m.leather,local([0,s*(.25+i*.031),.165-i*.008]),[.055,.013,.05]);
      combineStatic(limb);bowLimbs.push(limb);
    }
    ...
    arrow=new THREE.Group();root.add(arrow);rod(arrow,m.wood,[0,0,-.08],[0,0,.70],.005);
    const tip=mesh(arrow,new THREE.ConeGeometry(.018,.068,4),m.edge,[0,0,.73]);tip.rotation.x=Math.PI/2;
    for(const s of [-1,1]){const feather=plate(arrow,m.cloth,[[-.018,-.065],[.018,-.065],[.028,.035],[0,.015]],.002);feather.rotation.x=Math.PI/2;feather.rotation.y=s*Math.PI/3;}
    support=[0,0,.2];root.userData.projectileSocket=[0,0,.2];
    ...
    const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(new Float32Array([-.37,0,.19,0,0,.02,.37,0,.19]),3));string=new THREE.Line(g,new THREE.LineBasicMaterial({color:'#978a70'}));root.add(string);
    arrow=new THREE.Group();root.add(arrow);rod(arrow,m.wood,[0,.055,.02],[0,.055,.52],.006);
    const tip=mesh(arrow,new THREE.ConeGeometry(.015,.055,4),m.edge,[0,.055,.55]);tip.rotation.x=Math.PI/2;
    for(const s of [-1,1])rivet(root,m.trim,s*.026,.043,.16,.009);support=[0,-.045,.16];
    root.userData.projectileSocket=[0,.055,.36];
  }
  if(two&&!support)support=[0,-.2,0];
  if(arrow)combineStatic(arrow);
  root.userData.primary=category==='crossbow'?[0,-.07,-.065]:[0,0,0];
  root.userData.support=support;root.userData.category=category;root.userData.string=string;root.userData.arrow=arrow;root.userData.bowLimbs=bowLimbs;
  return combineStatic(root,new Set([string,arrow,...bowLimbs]));
}
