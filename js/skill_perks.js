...
const SkillPerks = (() => {
  ...
  const profiles = {
    ...
    summon: s => [option("Savage Bond",mul("minionStats.dmg",1.2)),economy(),option("Stalwart Bond",mul("minionStats.hp",1.25)),option("Growing Host",add("cap",1)),option("Frenzied Bond",mul("minionStats.atkRate",1.25)),option("Hunting Bond",mul("minionStats.speed",1.3),mul("minionStats.hp",1.15))],
    summon_golem: () => [power("slamDmg"),economy(),option("Dense Stitching",mul("golemHp",1.25)),option("One More Soul",add("maxCorpses",1)),option("Bone Colossus",mul("golemHp",1.4)),option("Gather the Fallen",mul("gatherRadius",1.5),mul("growDmg",1.5))],
  ...
  function effectText(e) {
    ...
    if(["baseThresh","pyreChance"].includes(root)&&e.op === "add")return `+${number(e.value*100)} percentage points to ${labels[root]}`;
  ...
  function describe(sk,rk) {
    ...
    for(const key of (metricPaths[sk.type]||"").split(" ")) {
      ...
      const v=valueAt(sk,key,rk); if(v===undefined || typeof v==="number"&&v===0)continue;
      ...
      const seconds=new Set(["dur","ttl","projTtl","weaponTtl","chill","freeze","delay","tickEvery","tickRate","maxChannel","timer","rallyDur","stunDur","stun","shredDur","tempoDuration","parryWindow","debuffDuration","orbTick","fieldTtl","maxDraw","root","lifetime","zapCd","pulseCd","wispCd","tickCd","trapArm","trapTtl","coatDuration","slowDur"]);
