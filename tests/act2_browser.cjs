...
const enemy=process.argv.includes('--enemies'),port=enemy?8748:8746,output=enemy?'act2_enemies':'act2_redesign';
(async()=>{
 ...
 try{
  for(const target of process.argv.slice(2).filter(x=>!x.startsWith('--'))){
   ...
   await page.waitForFunction(()=>window.act2ReviewDone||document.body.dataset.testStatus==='failed'||(!location.pathname.endsWith('act2_review.html')&&document.body.dataset.testStatus==='passed'),null,{timeout:1800000});
