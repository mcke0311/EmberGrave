...
const U = {
  ...
  astar(walk, w, h, sx, sy, tx, ty, maxExpand, elev, maxStep, traversal) {
    ...
    const came = new Map(), gs = new Map(), kinds = new Map(), closed = new Set();
    const key = (x, y) => x + y * w;
    ...
    while (open.size() > 0 && expand < maxExpand) {
      ...
      closed.add(ck); expand++;
      if (cur.x === tx && cur.y === ty) {
        const path = []; let k = key(tx, ty);
        let cx = tx, cy = ty;
        while (k !== key(sx, sy)) {
          path.push(traversal ? { x: cx, y: cy, kind: kinds.get(k) } : { x: cx, y: cy });
          const p = came.get(k); if (p === undefined) break;
          cx = p % w; cy = (p / w) | 0; k = p;
        }
      ...
      for (const d of DIRS) {
        ...
        const edge = traversal ? traversal(cur.x, cur.y, nx, ny, d[2]) : {cost: d[2]};
        if (!edge) continue;
        const ng = cg + edge.cost;
        const nk = key(nx, ny);
        if (gs.has(nk) && gs.get(nk) <= ng) continue;
        gs.set(nk, ng); came.set(nk, key(cur.x, cur.y)); kinds.set(nk, edge.kind);
