...
const Sfx = (() => {
  ...
  function init() {
    if (ac) return Promise.all([ac.state === "suspended" ? ac.resume().catch(() => {}) : null, loadClick(), loadDeath()]);
  ...
  function play(name) { if (name === 'click') { playClick(); return; } if (name === 'death') { playDeath(); return; } if (!ac || ac.state !== "running") return; try { (S[name] || (() => {}))(); } catch (e) {} }
  ...
  fetchDeath();
  return { init, play, voice, music, chooseZoneMusic, stopMusic, stopDeath, setVol, vol, TRACKS, EFFECTS, get ctx() { return ac; } };
})();
