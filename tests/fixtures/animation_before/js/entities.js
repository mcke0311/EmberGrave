...
class Entity {
  ...
  updateTraversal(dt) {
    ...
    const lift = (x, y) => (map.elev?.[Math.floor(x) + Math.floor(y) * map.w] || 0) * 14;
...
class Player extends Entity {
  ...
  computeStats() {
    const g = this.gearStats(), p = this.passiveStats();
    const b = {};
  ...
  effRank(id) {
    ...
    if (rk <= 0) return 0;
    const st = this.stats; if (!st) return rk;
  ...
  tryBlock() {
    return this.stats.block > 0 && Math.random() * 100 < this.stats.block;
  }
  ...
  performSkill(skillId, target, point) {
    ...
    const sk = isBasic ? DATA.BASIC_ATTACK : DATA.SKILLS[skillId];
    ...
    switch (sk.type) {
      case "melee": {
        ...
        if (this.stats.ranged) {
          Game.afterDelay(dur * 0.45, () => {
            if (this.dead) return;
            Sfx.play("bow");
            Game.spawnProjectile({ x: this.x, y: this.y, tx: target.x, ty: target.y, speed: 11, kind: "arrow", fromPlayer: true, mult, quarryOnHit: sk.quarryOnHit });
          });
        } else {
          Sfx.play("swing");
          Game.afterDelay(dur * 0.5, () => {
      ...
      case "leap": {
        ...
        if (!MapGen.walkable(Game.state.map, px, py)) { Game.msg("No footing there.", "#9a9a9a"); return false; }
        this.pay(sk, rk);
      ...
      case "wfan": case "wpierce": {
        ...
        Game.afterDelay(dur * 0.45, () => {
          if (this.dead) return;
          Sfx.play("bow");
          const base = { x: this.x, y: this.y, speed: 12, kind: "arrow", fromPlayer: true, mult, quarryOnHit: sk.quarryOnHit };
          if (sk.type === "wfan") {
      ...
      case "trap": {
        ...
        const trap = {
          ...
          armT: 0.7, ttl: 24, radius: sk.radius(rk), trigger: 1.25,
        ...
        if (Game.state.traps.length >= 4) Game.state.traps.shift();
      ...
      case "projectile": case "pierce": case "chain": case "fan": {
        ...
        Game.afterDelay(dur * 0.55, () => {
          ...
          const mkSpell = () => { const r = roll(); return { dmg: r.dmg, crit: r.crit, elem: sk.elem, burn, chill, pdot: sk.pdot ? sk.pdot(rk) : 0, drain: sk.drain ? sk.drain(rk) : 0, pierce: sk.type === "pierce", chains: sk.type === "chain" ? sk.jumps(rk) : 0, scorch: sk.scorch ? sk.scorch(rk) : 0, static: sk.buildStatic ? 1 : 0 }; };
          if (sk.type === "fan") {
            const n = sk.count(rk), spread = sk.spread(rk);
            const a0 = Math.atan2(aim.y - this.y, aim.x - this.x);
            for (let i = 0; i < n; i++) {
      ...
      case "fireclaw": {
        ...
        if (target && !target.dead) {
          ...
          Game.afterDelay((1 / this.stats.attackRate) * 0.4, () => {
            if (this.dead || target.dead) return;
        ...
        for (let i = 0; i < N; i++) {
          ...
          if (i === 0) boom(cx, cy); else Game.afterDelay(i * DUR, () => boom(cx, cy));
        }
        return true;
      }
      ...
      case "meteor": {
        ...
        Game.afterDelay(dur * 0.5, () => { if (!this.dead) Sfx.play("firebolt"); });
        Game.afterDelay(dur * 0.5 + delay, () => {
          if (this.dead) return;
          Sfx.play(sk.sound || "blast");
      ...
      case "combo": {   // Tempo Strike — a weapon hit that builds Tempo
        ...
        const mult = sk.dmgMult(rk) * syn;
        Game.afterDelay(dur * 0.5, () => { if (this.dead || target.dead) return; if (U.dist(this.x, this.y, target.x, target.y) <= this.stats.range + target.radius + 0.4) this.strike(target, mult); });
        const cap = 3 + (this.stats.tempoCapBonus || 0);
        this.tempo = Math.min(cap, (this.tempo || 0) + (this.stance === "berserk" ? 2 : 1));
        this.tempoUntil = Game.state.time + 4 + (this.stats.tempoWindowBonus || 0);
        return true;
      }
      ...
      case "shout": {   // Rallying Cry — heal + cleanse + run
        ...
        this.computeStats();
        for (const mi of Game.state.minions) if (!mi.dead && U.dist(this.x, this.y, mi.x, mi.y) < sk.radius(rk)) mi.hp = Math.min(mi.maxHp, mi.hp + mi.maxHp * 0.25);
      ...
      case "fear": {   // Terrifying Bellow (load-bearing id terrifying_bellow)
        ...
        for (const mon of Game.state.monsters) { if (mon.dead || U.dist(this.x, this.y, mon.x, mon.y) > R + mon.radius) continue; if (mon.isBoss) mon.applySlow(sk.dur(rk), 50); else { mon.feared = Game.state.time + sk.dur(rk); mon.aggro = true; } }
        return true;
      }
      case "combat_stance": case "parry_stance": {   // Berserker / Bulwark / Riposte (toggle)
        ...
        if (sk.type === "parry_stance") { this.riposteData = { mult: sk.riposteMult(rk), stun: sk.stunDur(rk), drain: sk.drainPerSec(rk), refund: 4, window: 0.6 }; this.buffs.push({ id: bid, label: sk.name, emoji: "🜛", stats: {}, until: Game.state.time + 99999 }); }
      ...
      case "banner": case "banner_ultimate": {   // War Banner / Standard of the Last Stand
        ...
        Game.state.fx.push({ type: "banner", tag: sk.id, x: aim.x, y: aim.y, radius: sk.radius(rk), ttl: sk.dur(rk), maxTtl: sk.dur(rk), tickEvery: 0.3,
          ...
          heal: sk.type === "banner_ultimate" ? (sk.heal ? sk.heal(rk) : 4) : 0, kindCol: sk.type === "banner_ultimate" ? "#ffe6a0" : "#d8b84a", owner: this });
      ...
      case "warshout_debuff": {   // Sundering Roar — strip + shred + slow, no damage
        ...
        for (const mon of Game.state.monsters) { if (mon.dead || U.dist(this.x, this.y, mon.x, mon.y) > R + mon.radius) continue; mon.sunder = { until: Game.state.time + 6, stacks: Math.min(3, ((mon.sunder && mon.sunder.stacks) || 0) + 2) }; mon.applySlow(6, 15); }
        return true;
      }
      /* ===================== EMBER WITCH ===================== */
      ...
      case "pyreblast": {   // Pyre — detonation that devours Scorch stacks
        ...
        const chainCh = 0.10 + 0.02 * rk;   // per Scorch stack devoured, chance to erupt a second Pyre on the spot
        const blast = (at, allowChain) => {
          ...
          if (allowChain && consumed > 0) {   // roll once per devoured stack; each success re-detonates here (follow-ups don't re-chain)
            ...
            for (let e = 0; e < extra; e++) Game.afterDelay(0.18 * (e + 1), () => blast(at, false));
          }
        };
        Game.afterDelay(dur0 * 0.55, () => blast(spot, true));
        return true;
      }
      ...
      case "balllightning": {   // Ball Lightning — slow drifting plasma orb that discharges arcs
        ...
        Game.afterDelay(dur0 * 0.5, () => { if (this.dead) return; Sfx.play("zap"); Game.state.fx.push({ type: "cyclone", orb: true, x: bx, y: by, vx: Math.cos(a) * 2.5, vy: Math.sin(a) * 2.5, ttl: sk.dur(rk), radius: 3.0, tickEvery: 0.35, drift: 0, lo: dm[0], hi: dm[1], pull: 0, owner: this }); });
        return true;
      }
      case "arcblink": {   // Arc Teleport — blink then burst on arrival
        ...
        this.x = tx; this.y = ty; this.path = null; Sfx.play("zap");
        const radius = sk.radius(rk);
      ...
      case "groundfield": {
        ...
        Game.afterDelay(dur0 * 0.5, () => { if (this.dead) return; Sfx.play(kind === "glacier" ? "frost" : kind === "static" ? "zap" : "blast"); Game.addNova(aim.x, aim.y, f.radius, col); Game.state.fx.push(f); });
        return true;
      }
      /* ===================== GRAVEBINDER ===================== */
      ...
      case "taunt_curse": {   // Hex of Beckoning — stampede to a mark
        ...
        Game.afterDelay(dur0 * 0.5, () => { if (this.dead) return; Sfx.play("curse"); Game.addNova(spot.x, spot.y, radius, "#9a40c0"); for (const mon of Game.state.monsters) { if (mon.dead || U.dist(spot.x, spot.y, mon.x, mon.y) > radius + mon.radius) continue; mon.beckon = { until: Game.state.time + dur, x: spot.x, y: spot.y }; } });
        return true;
      }
      ...
      case "devour": {   // Devour Corpse — heal + cleanse
        ...
        let corpse = null, bd = 81; for (const mon of Game.state.monsters) { if (!mon.dead || mon.corpseT <= 0 || mon.exploded) continue; const dd = U.dist2(aim.x, aim.y, mon.x, mon.y); if (dd < bd) { bd = dd; corpse = mon; } }
        if (!corpse) corpse = Game.corpseFromGrave(this, aim, 9);
        ...
        for (const mi of Game.state.minions) if (!mi.dead && U.dist(this.x, this.y, mi.x, mi.y) < 4) mi.hp = Math.min(mi.maxHp, mi.hp + mi.maxHp * 0.1);
      ...
      case "outbreak": {   // Outbreak — expanding pestilence ring
        ...
        Game.afterDelay(dur0 * 0.5, () => { if (this.dead) return; Sfx.play("blast"); Game.state.fx.push({ type: "outbreak", x: bx, y: by, r: 0, maxR: sk.maxR(rk), dur: sk.dur(rk), ttl: sk.dur(rk) + 0.5, tick: sk.tick(rk), corpseDmg: sk.corpseDmg(rk), hitMon: new Set(), owner: this }); });
        return true;
      }
      case "summon_golem": {   // Bone Golem — stitched from corpses; re-cast on corpses to feed & grow
        ...
        const recompute = mi => {
          ...
          mi.maxHp = Math.floor((100 + 30 * rk + sk.growHp(rk) * (st - 1)) * (1 + this.stats.minionHpPct / 100)); mi.hp = mi.maxHp;
      ...
      case "ricochet": {   // Ricochet Shard — bank-shot arrow
        ...
        Game.afterDelay(dur * 0.4, () => { if (this.dead) return; Game.spawnProjectile({ x: this.x, y: this.y, tx: aim.x, ty: aim.y, speed: 14, kind: "arrow", fromPlayer: true, mult: sk.dmgMult(rk) * syn, ricochet: { bounces: sk.bounces(rk) } }); });
        return true;
      }
      ...
      case "decoy": case "afterimage": {   // Snare Decoy / After-Image — a taunting lure
        ...
        if (sk.type === "afterimage") {
          ...
          const nx = this.x + Math.cos(dir) * rr, ny = this.y + Math.sin(dir) * rr; if (MapGen.walkable(Game.state.map, nx, ny)) { this.x = nx; this.y = ny; this.path = null; }
        }
      ...
      case "totem": {   // Storm Totem / Tempest Totem
        ...
        this.pay(sk, rk);
        const kind = sk.totemKind || "storm", ttl = sk.ttl(rk) * (1 + (this.stats.totemTtl || 0)), dm = sk.dmg(rk);
        const cap = sk.cap(rk) + Math.floor((this.effRank("totem_mastery") || 0) / 10);   // Totem Mastery: +1 totem per 10 ranks
  ...
  update(dt) {
    ...
    if (this.stance === "bulwark") this.rootT = this.moving ? 0 : Math.min(3, this.rootT + dt);
    ...
    if (this.dashing) {
      ...
      if (d <= st.range + t.radius) {
        ...
        Game.afterDelay(0.18, () => { if (!t.dead && !this.dead) this.strike(t, D.mult); });
      } else {
    ...
    if (cmd.type === "move") {
      ...
      const skDef = cmd.skill === "basic" ? DATA.BASIC_ATTACK : DATA.SKILLS[cmd.skill];
  ...
  update(dt, player, map) {
    ...
    if (this.charging) {
      ...
      if (!C.hit && !player.dead && U.dist(this.x, this.y, player.x, player.y) < player.radius + this.radius + 0.3) {
        if (!player.tryBlock()) player.takeDamage(U.rf(...this.def.dmg) * (this.def.charge.mult || 1.6) * (this.def.dmgMult || 1) * 2.2 * this.witherMult(), this);
    ...
    if (inRange && !tgt.dead) {
      ...
      if (this.attackCd <= 0) {
        ...
        Game.afterDelay(dur * 0.55, () => {
          ...
          if (tgt.tryBlock && tgt.tryBlock()) { Sfx.play("block"); Game.addFloat(tgt.x, tgt.y, "block", "#80a0ff"); return; }
      ...
      if (this.fromPlayer) {
        for (const m of monsters) {
          ...
          if (U.dist(this.x, this.y, m.x, m.y) < m.radius + 0.25) {
            ...
            if (this.quarryOnHit) m.quarry = { until: Game.state.time + 6, stacks: Math.min(5, ((m.quarry && m.quarry.stacks) || 0) + 1) };
            if (player.coat && player.buffs.some(b => b.id === "serrated")) m.poisonDot = { dps: Math.max((m.poisonDot && m.poisonDot.dps) || 0, player.coat.pdot / 3), t: 4 };
        ...
        if (!player.dead && U.dist(this.x, this.y, player.x, player.y) < player.radius + 0.25) {
          ...
          else if (player.tryBlock()) { Sfx.play("block"); Game.addFloat(player.x, player.y, "block", "#80a0ff"); }
          else if (mon) {
