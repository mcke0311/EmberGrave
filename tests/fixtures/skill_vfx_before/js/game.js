...
const Game = (() => {
  ...
  async function enterMap(zoneId, spawnKey, { revive = false, openingMode = null } = {}) {
    ...
    Sfx.stopDeath(); // A death cry must not carry into the respawned hero's town.
  ...
  function addNova(x, y, radius, color) { novas.push({ x, y, radius, color, t: 0, dur: 0.35 }); if (radius >= 1.2) breakPropsNear(x, y, radius * 0.9); }   // AoE skills shatter breakables
  /* a jagged lightning arc between two world points (Arc Lattice, Thunderstorm) */
  function lightningBolt(x0, y0, x1, y1, color) {
    bolts.push({ x0, y0, x1, y1, color: color || "#fff080", t: 0, dur: 0.22, seed: (Math.random() * 1000) | 0 });
  }
  /* a straight glowing beam between two world points (beam-type skills) */
  function beamFx(x0, y0, x1, y1, color) {
    bolts.push({ x0, y0, x1, y1, color: color || "#fff080", t: 0, dur: 0.2, seed: 0, straight: true, width: 6.5 });
  }
  function spawnProjectile(o) {
    ...
    state.projectiles.push(new Projectile(o));
  }
  ...
  function placeEvents(map) {
    ...
    for (let n = 0; n < count; n++) {
      ...
      for (let tries = 0; tries < 70 && !ok; tries++) {
        ...
        if (MapGen.walkable(map, x, y) && U.dist(x, y, sp.x, sp.y) > 9) ok = true;
      }
      if (!ok) continue;
      if (ev.kind === "goblin") {
  ...
  function onPlayerDeath(source) {
    ...
    p.command = null; p.path = null; heldTarget = null;
    ...
    if (p.hardcore) {
      ...
      afterDelay(3.5, () => { running = false; state = null; Sfx.stopDeath(); Sfx.stopMusic(); UI.closeAll(); UI.showTitle(); });
    } else {
  ...
  function bindInput() {
    ...
    window.addEventListener("mouseup", e => {
      if (e.button === 0) { mouse.l = false; heldTarget = null; }
    ...
    window.addEventListener("blur", () => { mouse.l = mouse.r = mouse.shift = mouse.alt = false; LootFilter.setReveal(false); });
  }
  ...
  function handleClick(rightBtn) {
    const p = state.player;
    if (p.dead) return;
    if (p.jumping) { const world=screenToWorld(mouse.x,mouse.y);if(world)p._pendingClick = {rightBtn, mouse: {...mouse}, world}; return; }
    ...
    if (mouse.shift || rightBtn) {
      ...
      if (sk.type === "melee") {
        ...
        if (p.stats.ranged) {
          ...
          spawnProjectile({ x: p.x, y: p.y, tx: w.x, ty: w.y, speed: 11, kind: "arrow", fromPlayer: true, mult, quarryOnHit: sk.quarryOnHit, quarryStacks:sk.quarryStacks?.(rk2), pierce:!!sk.perkPierce });
        } else Sfx.play("swing");
      } else {
  ...
  function heldUpdate() {
    const p = state.player;
    if (p.dead || UI.cursorItem) return;
    ...
    if (mouse.l && !mouse.r) {
      if (options.leftClickMove && !mouse.shift) {
        /* move-only: hold left to keep walking to the cursor, never lunge at foes */
        if (!hoverLabel && !hoverNpc && !hoverProp && !hoverExit) {
          const w = screenToWorld(mouse.x, mouse.y);
          if (w&&(!p.command || p.command.type === "move")) { p.command = { type: "move" }; repath(p, w.x, w.y); }
        }
      } else if (heldTarget && !heldTarget.dead && repeatSkill(p.skillL)) {
        p.command = { type: "attack", target: heldTarget, skill: p.skillL, hold: true };
      } else if (!hoverMon && !hoverLabel && !hoverNpc && !hoverProp && !hoverExit && !mouse.shift) {
        const w = screenToWorld(mouse.x, mouse.y);
        if (w&&(!p.command || p.command.type === "move")) {
          p.command = { type: "move" };
          repath(p, w.x, w.y);
        }
      } else if (hoverMon && !hoverMon.dead) {
        heldTarget = hoverMon;
      }
    }
  ...
  function monsterGeometry(mon, cam) {
    ...
    if (mon.beacon) return SpriteAssets.frameGeometry(SpriteAssets.getFrame(SpriteAssets.maps.props.beacon, 0), x, y);
  ...
  function updateTraps(dt) {
    ...
    for (let i = state.traps.length - 1; i >= 0; i--) {
      ...
      Sfx.play(tr.kind === "powder" ? "blast" : tr.kind === "frost" ? "frost" : "hit");
      addNova(tr.x, tr.y, tr.radius, col);
  ...
  function updateTotem(f, o, dt) {
    if (f.totemKind === "tempest") {
      ...
      if (f.wispT <= 0) { f.wispT = f.wispCd || 3; let t = null, bd = 1e9; for (const m of state.monsters) { if (m.dead) continue; const dd = U.dist2(f.x, f.y, m.x, m.y); if (dd < bd) { bd = dd; t = m; } } if (t) state.fx.push({ type: "wisp", x: f.x, y: f.y, target: t, ttl: 3, lo: f.lo, hi: f.hi, owner: o }); }
    } else {
  ...
  function render() {
    ...
    for (const f of state.fx) if(f.type!=="slamwarning")drawFx(f, cam);
      ...
      switch (d.kind) {
        ...
        case "proj": {
          ...
          if (pr.kind === "firebolt") {
            ...
            ctx.moveTo(jx, jy);
            for (let i = 1; i <= 3; i++) {
              jx = d.sx + Math.cos(a) * (i * 6 - 9) + U.rf(-2.5, 2.5);
              jy = d.sy - 14 + Math.sin(a) * (i * 6 - 9) + U.rf(-2.5, 2.5);
              ctx.lineTo(jx, jy);
            }
            ctx.stroke();
            ctx.shadowBlur = 0;
            ...
            ctx.restore();
            if (Math.random() < 0.5) addParticle(pr.x, pr.y, "#90ff70");
            ...
            ctx.restore();
            if (Math.random() < 0.4) addParticle(pr.x, pr.y, "#9fe0c0");
          } else {
    ...
    for (const d of draws) {
      if (d.kind !== "mon" || d.mon.dead || d.mon === hoverMon) continue;
    ...
    if (state.bossBar) {
      ...
      const bx = bossCenter - bw / 2, by = 46;
      ...
      ctx.fillText(b.name + (b.def.title ? " — " + b.def.title : ""), bossCenter, by - 8);
  ...
  function renderLighting(cam) {
    ...
    const punch = (wx, wy, r, intensity, flicker) => {
      ...
      lightCtx.drawImage(lightMask, sx - rr, sy - 10 - rr, rr * 2, rr * 2);
  ...
  function rebuildMm() {
    const m = state.map;
    if (!mmCanvas || mmCanvas.width !== m.w) {
    ...
    /* mask unexplored */
    const img = c.getImageData(0, 0, m.w, m.h);
    for (let i = 0; i < m.explored.length; i++) {
      if (!m.explored[i]) img.data[i * 4 + 3] = 0;
    }
