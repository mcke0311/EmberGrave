...
const mode=process.argv[2]||'smoke',zone=process.argv[3]||'ash_wastes',out='tests/qa/cinders';
...
(async()=>{
 ...
 try{
  ...
  await page.goto('http://127.0.0.1:8875/tests/cinders_review.html?'+params,{waitUntil:'load',timeout:60000});
  ...
  if(mode==='capture'||mode==='profile'){
   for(let k=0;k<240;k++){
    ...
    if(mode==='capture'&&progress.text.startsWith('Saved ')||mode==='profile'&&progress.text.includes('performance comparison complete')){if(progress.text.startsWith('FAIL'))throw Error('Performance target exceeded; see performance_'+zone+'_both.json');break;}
   ...
   for(const id of ['ash_wastes','cinder_bastion','throne','hellgate']){
    const report=await page.evaluate(async id=>{
     ...
     if(after.totalBuilds!==before.totalBuilds||after.surfaceViewBuilds!==before.surfaceViewBuilds)throw Error('Warmed cache rebuilt');
     return {zone:id,props:m.props.length,enemies:m.monsterSpawns.length,landmarks:m.composition.landmarks.length,cacheStable:true};
