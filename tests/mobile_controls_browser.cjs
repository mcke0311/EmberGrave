...
(async()=>{
  ...
  try {
    ...
    await reveal(page.locator('#skillPick button').first());await page.locator('#skillPick button').first().tap();
    ...
    await page.locator('.phone-paged:not(.hidden):not(.workspace-inactive) .phone-pager-head .pclose').tap();
    ...
    await reveal(page.locator('#escmenu').getByRole('button',{name:'Back',exact:true,includeHidden:true}));await page.locator('#escmenu').getByRole('button',{name:'Back',exact:true,includeHidden:true}).tap();await reveal(page.getByRole('button',{name:'Resume',exact:true,includeHidden:true}));await page.getByRole('button',{name:'Resume',exact:true,includeHidden:true}).tap();
    ...
    ok(panel.y>=0&&panel.y+panel.height<=390,'landscape panel is clipped');await page.locator('.phone-paged:not(.hidden):not(.workspace-inactive) .phone-pager-head .pclose').tap();
