...
for(const zone of zones){
  ...
  for(const seed of seeds){
    ...
    for(const exit of m.exits){
      ...
      for(let dy=-2;dy<=4;dy++)for(let dx=-1;dx<=1;dx++)ok(!m.blocked[gx+dx+(gy+dy)*m.w],label+' blocked passage '+th.id+' '+[gx+dx,gy+dy]+' opening '+JSON.stringify(th.opening));
