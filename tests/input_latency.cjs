...
(async () => {
  ...
  const report = {environment: {browser: browser.version(), platform: process.platform}, zone, key, width, height, seconds, warmupMs, terrainVersion, results: []};
  try {
    ...
    await page.route('**/js/game.js*', async route => {
      let source = fs.readFileSync(path.join(__dirname, '../js/game.js'), 'utf8').replace(/\r\n/g, '\n');
      ...
      replace('    init, newGame, loadGame,', `    __latency: {settle(seconds){state.time+=seconds;for(let i=delayed.length-1;i>=0;i--)if(state.time>=delayed[i].t){const fn=delayed[i].fn;delayed.splice(i,1);fn();}},place(x,y) {state.player.x=x;state.player.y=y;state.player.path=null;state.player.command=null;camPos=null;fx.shake=0;}},\n    init, newGame, loadGame,`);
    ...
    for (const mode of modes) {
      ...
      const scene = await page.evaluate(async ({zone, mode, boss}) => {
        ...
        if(boss){
          ...
          if(boss==='vethriss')e.memoryIndex=3;
      ...
      if (raw.fatal || errors.length || !raw.titleHidden || !raw.frames.length) throw Error(raw.fatal || errors.join(';') || 'Scene did not render: '+JSON.stringify({frames:raw.frames.length,titleHidden:raw.titleHidden,scene}));
