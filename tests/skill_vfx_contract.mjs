...
const currentGameplay=layered||process.argv.includes('--current-gameplay');
...
function harness({before=false,enabled=true}={}){
  ...
  function fresh(sk,rank=10,perk=-1){
    ...
    if(layered){
      s.map.elev=new Uint8Array(1600);s.map.surfaceVersion=1;s.map.ramps=[];
      s.map.layers={1:{...s.map,elev:new Uint8Array(1600).fill(8)}};s.map.surfaceLinks=[];
      p.surfaceId=1;
    }
    ...
    if(layered){
      for(const m of s.monsters)m.surfaceId=1;
      for(const original of [...s.monsters]){
        const lower=new api.Monster(enemyId,original.x,original.y);lower.surfaceId=0;lower.hp=lower.maxHp=1e7;
        if(original.dead){lower.dead=true;lower.hp=0;lower.corpseT=20;}
        lower.floorSentinel=true;s.monsters.push(lower);
      }
      s.lowerBefore=JSON.stringify(s.monsters.filter(m=>m.floorSentinel).map(m=>({hp:m.hp,dead:m.dead,exploded:m.exploded,stun:m.stunT,slow:m.slowT,poison:m.poisonDot,scorch:m.scorch,doom:m.doom,plague:m.plague,mark:m.killMark})));
    }
  ...
  function step(s,p,seconds){for(let i=0;i<Math.round(seconds*60);i++){
    ...
    if(layered){G.__test.updateLayerEffects('traps',G.__test.updateTraps,1/60);G.__test.updateLayerEffects('fx',G.__test.updateFx,1/60);}
    else{G.__test.updateTraps(1/60);G.__test.updateFx(1/60);}
  ...
  function run(sk,rank,perk){
    const {p,s,target}=fresh(sk,rank,perk),result=sk.type==='passive'?(p.computeStats(),true):p.performSkill(sk.id,target,{x:11,y:11,surfaceId:p.surfaceId});
    step(s,p,2);
    if(layered){
      const below=JSON.stringify(s.monsters.filter(m=>m.floorSentinel).map(m=>({hp:m.hp,dead:m.dead,exploded:m.exploded,stun:m.stunT,slow:m.slowT,poison:m.poisonDot,scorch:m.scorch,doom:m.doom,plague:m.plague,mark:m.killMark})));
      assert.equal(below,s.lowerBefore,sk.id+' affected lower-floor actors');
      assert.equal(p.surfaceId,1,sk.id+' changed floors without stairs');
      for(const mi of s.minions)assert.equal(mi.surfaceId,1,sk.id+' spawned a companion on the wrong floor');
    }
    return snapshot(p,s,result);
  }
  function lifecycle(id,mode='expire'){
    ...
    for(let frame=0;frame<720;frame++){
      s.time+=1/60;G.__test.flush();p.update(1/60);
      if(layered){G.__test.updateLayerEffects('traps',G.__test.updateTraps,1/60);G.__test.updateLayerEffects('fx',G.__test.updateFx,1/60);}
      else{G.__test.updateTraps(1/60);G.__test.updateFx(1/60);}
...
const original=currentGameplay?null:harness({before:true});
