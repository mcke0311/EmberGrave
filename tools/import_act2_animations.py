"""Pack approved transparent Act 2 sequences; originals remain unchanged.
...
REPORT = ROOT / 'assets/act2_animations/import.json'
    ...
    for spec in report['actors'].values():
        for key in ['source', 'output']:
            if digest(ROOT / spec[key]) != spec[key+'Hash']:
                raise ValueError('Act 2 art changed; rerun importer: '+spec[key])
...
def pack(spec):
    ...
    frames = cutouts(image, rows, spec['id'] in ['mire_mother_1','mire_mother_2'])
    ...
    target_height = 210 if spec['id'].startswith('mire_mother') else box[3]-box[1]
    cell_h=384 if spec['id'].startswith('mire_mother') else 256
    ground_y=cell_h-20
    ...
    output=ROOT/'assets/act2_animations/packed'/ (spec['id']+'.webp');output.parent.mkdir(parents=True,exist_ok=True)
    atlas.save(output,'WEBP',lossless=True,method=6)
    preview=ROOT/'tests/qa/act2_animation/previews';preview.mkdir(parents=True,exist_ok=True)
    ...
    entry={'kind':'atlas','src':output.relative_to(ROOT).as_posix(),'revision':digest(output)[:12], 'cell':[cell_w,cell_h],'cols':6,'rows':rows,'anchor':[ground_x,ground_y],'anchors':anchors,'bundle':'boss:mire_mother' if spec['id'].startswith('mire_mother') else 'actors:act2','act2Art':True,'isolateFrameSampling':True,'hitShapes':[actor_hit_shape(f) for f in packed]}
...
def main():
    ...
    if args.check:
        ...
        config=json.loads(CONFIG.read_text());expected={"actor.act2."+e['id'] for e in config['enemies']}
        assert set(entries)==expected,'Incomplete Act 2 atlas roster'
    ...
    for spec in config['enemies']:
        ...
        entry,actor=pack(spec);report['entries']['actor.act2.'+spec['id']]=entry;report['actors'][spec['id']]=actor
