"""Pack approved transparent Act 5 sequences; originals remain unchanged.
...
CONFIG = ROOT / 'assets/act5_animations/generated.json'
REPORT = ROOT / 'assets/act5_animations/import.json'
    ...
    for spec in report['actors'].values():
        for key in ['source', 'output']:
            if digest(ROOT / spec[key]) != spec[key+'Hash']:
                raise ValueError('Act 5 art changed; rerun importer: '+spec[key])
...
def pack(spec):
    ...
    output=ROOT/'assets/act5_animations/packed'/ (spec['id']+'.webp');output.parent.mkdir(parents=True,exist_ok=True)
    atlas.save(output,'WEBP',lossless=True,method=6)
    preview=ROOT/'tests/qa/act5_animation/previews';preview.mkdir(parents=True,exist_ok=True)
    ...
    entry={'kind':'atlas','src':output.relative_to(ROOT).as_posix(),'revision':digest(output)[:12], 'cell':[cell_w,cell_h],'cols':6,'rows':rows,'anchor':[ground_x,ground_y],'anchors':anchors,'bundle':'actors:act5','act5Art':True,'sequences':spec['sequences'],'isolateFrameSampling':True,'hitShapes':[actor_hit_shape(f) for f in packed]}
...
def main():
    ...
    if args.check:
        ...
        config=json.loads(CONFIG.read_text(encoding='utf-8-sig'));expected={"actor.act5."+e['id'] for e in config['enemies']}
        assert set(entries)==expected,'Incomplete Act 5 atlas roster'
    ...
    config=json.loads(CONFIG.read_text(encoding='utf-8-sig'));report=json.loads(REPORT.read_text()) if REPORT.exists() else {'version':1,'actors':{},'entries':{}}
    for spec in config['enemies']:
        if args.install_only:continue
        if args.only and spec['id'] not in args.only:continue
        print('Packing',spec['id'],flush=True)
        entry,actor=pack(spec);report['entries']['actor.act5.'+spec['id']]=entry;report['actors'][spec['id']]=actor
