"""Pack approved transparent Act 4 sequences; originals remain unchanged.
...
CONFIG = ROOT / 'assets/act4_animations/generated.json'
REPORT = ROOT / 'assets/act4_animations/import.json'
    ...
    for spec in report['actors'].values():
        for key in ['source', 'output']:
            if digest(ROOT / spec[key]) != spec[key+'Hash']:
                raise ValueError('Act 4 art changed; rerun importer: '+spec[key])
...
def pack(spec):
    ...
    output=ROOT/'assets/act4_animations/packed'/ (spec['id']+'.webp');output.parent.mkdir(parents=True,exist_ok=True)
    atlas.save(output,'WEBP',lossless=True,method=6)
    preview=ROOT/'tests/qa/act4_animation/previews';preview.mkdir(parents=True,exist_ok=True)
    ...
    entry={'kind':'atlas','src':output.relative_to(ROOT).as_posix(),'revision':digest(output)[:12], 'cell':[cell_w,cell_h],'cols':6,'rows':rows,'anchor':[ground_x,ground_y],'anchors':anchors,'bundle':'actors:act4','act4Art':True,'isolateFrameSampling':True,'hitShapes':[actor_hit_shape(f) for f in packed]}
...
def main():
    ...
    if args.check:
        ...
        config=json.loads(CONFIG.read_text());expected={"actor.act4."+e['id'] for e in config['enemies']}
        assert set(entries)==expected,'Incomplete Act 4 atlas roster'
    ...
    for spec in config['enemies']:
        ...
        entry,actor=pack(spec);report['entries']['actor.act4.'+spec['id']]=entry;report['actors'][spec['id']]=actor
